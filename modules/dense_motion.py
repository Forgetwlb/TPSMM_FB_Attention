...
import torch
from modules.util import Hourglass, AntiAliasInterpolation2d, make_coordinate_grid, kp2gaussian, estimate_optical_flow
from modules.util import to_homogeneous, from_homogeneous, UpBlock2d, TPS, flow_warp_image
from modules.flow_visual import flow_to_image, flow2rgb,flow_to_image_torch,write_flo_file,deformed_source1
import math
...
class DenseMotionNetwork(nn.Module):
    ...
    Module that estimating an optical flow and multi-resolution occlusion masks 
                        from K TPS transformations and an affine transformation.从K-TPS变换和仿射变换估计光流和多分辨率遮挡的模块。
    """
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_tps, num_channels, 
        ...
        self.hourglass = Hourglass(block_expansion=block_expansion, in_features=(num_channels * (num_tps+1) + num_tps*5+1+3),
                                   max_features=max_features, num_blocks=num_blocks)

        hourglass_output_size = self.hourglass.out_channels
        self.maps = nn.Conv2d(hourglass_output_size[-1], num_tps + 1 + 1, kernel_size=(7, 7), padding=(3, 3))
    ...
    def create_transformations(self, source_image, kp_driving, kp_source, bg_param, optical_flow_grid):
        ...
        optical_flow_grid = optical_flow_grid.view(bs, -1, h, w, 2).to(kp_1.device)
        identity_grid = make_coordinate_grid((h, w), type=kp_1.type()).to(kp_1.device)
        identity_grid = identity_grid.view(1, 1, h, w, 2)
        identity_grid = identity_grid.repeat(bs, 1, 1, 1, 1)

        # affine background transformation 仿射背景变换
        ...
        transformations = torch.cat([identity_grid, driving_to_source, optical_flow_grid], dim=1)
        return transformations


    def flow_warp_grid(self, source_image, optical_flow):
    ...
    def create_deformed_source_image(self, source_image, transformations):
        ...
        bs, _, h, w = source_image.shape
        source_repeat = source_image.unsqueeze(1).unsqueeze(1).repeat(1, self.num_tps + 1 + 1, 1, 1, 1, 1)
        source_repeat = source_repeat.view(bs * (self.num_tps + 1 + 1), -1, h, w)
        transformations = transformations.view((bs * (self.num_tps + 1 +1), h, w, -1))
        deformed = F.grid_sample(source_repeat, transformations, align_corners=True)
        deformed = deformed.view((bs, self.num_tps+1+1, -1, h, w))
        return deformed
    ...
    def forward(self, source_image, driving_image, kp_driving, kp_source, bg_param = None,
                dropout_flag=False, dropout_p = 0):
        if self.scale_factor != 1:
            source_image = self.down(source_image)
            driving_image = self.down(driving_image)
        optical_flow = estimate_optical_flow(source_image, driving_image)
        # optical_flow_image = flow_warp_image(source_image, optical_flow)
        # 计算整体的光流平均值
        # mean_flow = torch.mean(optical_flow, dim=0)
        # mean_flow = mean_flow.cpu()
        # mean_flow = mean_flow.detach().numpy().astype('float32')
        # 将光流的方向映射到颜色编码
        # 计算光流的方向和大小
        # angle = np.arctan2(mean_flow[1, ...], mean_flow[0, ...])
        # magnitude = np.sqrt(mean_flow[0, ...] ** 2 + mean_flow[1, ...] ** 2)
        # # 对光流的强度进行归一化和调整范围
        # magnitude_normalized = (magnitude - np.min(magnitude)) / (np.max(magnitude))
        # hsv = np.zeros((mean_flow.shape[1], mean_flow.shape[2], 3), dtype=np.float32)
        # hsv[..., 0] = (angle * 180 / np.pi) / 2  # 方向映射到H通道
        #
        # # 将光流的强度映射到饱和度和亮度
        # # magnitude, _ = cv2.cartToPolar(mean_flow[0, ...], mean_flow[1, ...])
        # hsv[..., 1] = cv2.normalize(magnitude_normalized, None, 0, 255, cv2.NORM_MINMAX)  # 强度映射到饱和度
        #
        # # 将HSV图像转换为BGR图像
        ...
        # plt.imshow(rgb)
        # plt.axis('off')
        # plt.savefig('optical_flow_image.png')
        optical_flow_grid = self.flow_warp_grid(source_image, optical_flow.detach())
        bs, _, h, w = source_image.shape
        out_dict = dict()
        heatmap_representation = self.create_heatmap_representations(source_image, kp_driving, kp_source)
        transformations = self.create_transformations(source_image, kp_driving, kp_source, bg_param, optical_flow_grid)
        ...
        source_image0 = source_image[0]
        deformed_source0 = deformed_source
        ...
        image_all = deformed_source1(source_image0, deformed_source0, contribution_maps)
        image1 = (image_all * 255).astype(np.uint8)
        plt.imshow(image1)
        plt.axis('off')
        plt.savefig('image_all1.png')

        # Combine the K+1 transformations 结合K+1变换
        # Eq(6) in the paper
        ...
        occlusion_map_np = [tensor.cpu().detach().numpy() for tensor in occlusion_map]

        # Scale the values to the range [0, 255]
        occlusion_map_np_scaled = [(arr * 255).astype(np.uint8) for arr in occlusion_map_np]

        # Create an empty list to store PIL Image objects
        occlusion_map_images = []

        # Convert each numpy array to a PIL Image object
        for arr in occlusion_map_np_scaled:
            image = Image.fromarray(arr[0][0])
            occlusion_map_images.append(image)

        # Save each image as PNG
        for i, image in enumerate(occlusion_map_images):
            image.save(f"occlusion_map_{i + 1}.png")
...
if __name__ == '__main__':
    ...
    model = DenseMotionNetwork(64,5,1024,10,3)
    model = model.cuda()
