...
class GeneratorFullModel(torch.nn.Module):
    ...
    合并所有与生成器相关的更新到单个模型中，以便更好地使用多gpu
    """
    ...
    def forward(self, x, epoch):
        ...
        # image2 = image2 * 255
        # image2 = image2.cpu()
        # image2 = image2.detach().numpy().astype('uint8')
        # image2 = np.transpose(image2, (1, 2, 0))
        # plt.imshow(image2)
        # plt.axis('off')
        # plt.savefig('image_source0.png')
        # image3 = x['driving'][0]
        # image3 = image3 * 255
        # image3 = image3.cpu()
        # image3 = image3.detach().numpy().astype('uint8')
        # image3 = np.transpose(image3, (1, 2, 0))
        # plt.imshow(image3)
        # plt.axis('off')
        # plt.savefig('image_driving0.png')
        ...
        if (epoch >= self.dropout_epoch):
            ...
            # dropout_p will linearly increase from dropout_startp to dropout_maxp # Dropout_p将从dropout_start线性增加到dropout_maxp
            dropout_flag = True
            dropout_p = min(epoch / self.dropout_inc_epoch * self.dropout_maxp + self.dropout_startp, self.dropout_maxp)

        dense_motion = self.dense_motion_network(source_image=x['source'], driving_image=x['source'],
                                                 kp_driving=kp_driving, kp_source=kp_source,
                                                 dropout_flag=dropout_flag, dropout_p=dropout_p)
        generated = self.inpainting_network(x['source'], x['driving'], dense_motion)
        # generated = self.inpainting_network(x['source'], x['driving'], dense_motion)
        ...
        # if self.loss_weights['photometric_consistency_loss'] != 0:
        #
        #     driving_image = x['driving']
        #     flow_deformed_source = generated['flow_deformed_source']
        #     image1 = flow_deformed_source[0]
        #     image1 = image1 * 255
        #     image1 = image1.cpu()
        #     image1 = image1.detach().numpy().astype('uint8')
        #     image1 = np.transpose(image1, (1, 2, 0))
        #     plt.imshow(image1)
        #     plt.axis('off')
        #     plt.savefig('image_sampled_image2.png')
        #     # Compute pixel-wise photometric difference between image1 and warped_image2
        #     value = torch.abs(driving_image - flow_deformed_source).mean()
        #     loss_values['photometric_consistency_loss'] = self.loss_weights['photometric_consistency_loss'] * value
