...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_folder", default="MUG256_test/MUG256_test_014_anger_take000_gen", help="Folder with images")
    parser.add_argument("--out_file", default="gen.pkl", help="Extracted values")
    ...
    parser.add_argument("--type", default='face_id', choices=['face_id', 'face_pose', 'body_id', 'body_pose', 'vgg'],
                        help="Type of info to extract")

    args = parser.parse_args()
    # 这里使用了 locals() 函数来获取当前命名空间中的局部变量，通过拼接字符串构建函数名称。
    func = locals()["extract_" + args.type]
    out_file = args.out_file
    del args.type, args.out_file
    # vars(args) 返回 args 的属性字典，** 运算符用于将字典展开为关键字参数。
    df = func(**vars(args))
    # 最后，这行代码将 DataFrame 对象 df 使用 to_pickle 方法保存为 pickle 文件，文件路径由 out_file 变量指定。
