...
def make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network, device, mode = 'relative'):
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            dense_motion = dense_motion_network(source_image=source,driving_image=driving_frame,
                                                kp_driving=kp_norm,
                                                kp_source=kp_source, bg_param = None,
                                                dropout_flag = False)
            out = inpainting_network(source, driving_frame, dense_motion)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="log/nemo-256 22_06_23_08.44.27/nemo-256.yaml", help="path to config")
    parser.add_argument("--checkpoint", default='log/nemo-256 22_06_23_08.44.27/00000099-checkpoint.pth.tar', help="path to checkpoint to restore")
    parser.add_argument("--source_image", default='demo/0006.png', help="path to source image")
    parser.add_argument("--driving_video", default='nemo-256/train/005_spontaneous_smile_1', help="path to driving video")
    parser.add_argument("--result_video", default='two_test/0006_005', help="path to output")
    ...
    image_list = []
    driving_video = []
    for image_name in os.listdir(opt.driving_video):
        image_list.append(image_name)
    # 给图片序列排序
    image_list.sort(key=lambda x: int(x.split('.')[0]))
    # 给MUG数据集排序
    # image_list.sort(key=lambda x: int(x[4:8]))
    try:
        for im in image_list:
            im = os.path.join(opt.driving_video, im)
            reader = imageio.v2.imread(im)
            driving_video.append(reader)
    except RuntimeError:
        pass
    '''
    ...
    # imageio.mimsave(opt.result_video, [img_as_ubyte(frame) for frame in predictions],fps = 25)

# 生成PNG文件夹
    if not os.path.exists(opt.result_video):
        os.makedirs(opt.result_video)
    for i in range(len(predictions)):
        imageio.imsave(opt.result_video+'/'+str(i)+'.png', img_as_ubyte(predictions[i]))
